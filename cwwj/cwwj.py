...
def submit(users, loc):
    ...
    "guangzhou" : "sfzx=1&\
                ...
                city=%E5%B9%BF%E5%B7%9E%E5%B8%82&province=%E5%B9%BF%E4%B8%9C%E7%9C%81&\
        ...
        for t in range(3):
            try:
                ...
                if response.status_code == 200 and response.json()["e"] == 0:
                    ...
                    print(f"{user.userinfo[0]}上报失败")
                    raise RuntimeError(f"上报失败,{response.json()['m']}\n")
            except:
                continue
